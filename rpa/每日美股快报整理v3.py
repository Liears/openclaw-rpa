...
async def run():
    async with async_playwright() as p:
        ...
        try:
            ...
            # 步骤 2：搜索 NVDA
            try:
                _fill_target = 'Search box'
                _fill_value  = 'NVDA'
                _fill_loc = (
                    page.get_by_label(_fill_target) if await page.get_by_label(_fill_target).count() > 0
                    else page.get_by_placeholder(_fill_target) if await page.get_by_placeholder(_fill_target).count() > 0
                    else page.locator(f'input[name="{_fill_target}"]').first
                )
                await _fill_loc.fill(_fill_value)
