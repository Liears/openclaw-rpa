...
| 方向 | 说明 |
|:-----|:-----|
| **支持调用 API 的录制** | 除浏览器 DOM 操作外，把 **HTTP/API 调用**与第三方自动化接口纳入「录制 → 合成脚本 → 回放」同一套流程，便于对接更多外部服务。 |
| **可配置的自动登录 / 会话复用** | 通过 **凭证或持久化浏览器上下文** 等配置，让脚本在允许的前提下 **复用已登录会话**，减少每次回放都要重新「演一遍」复杂登录步骤；录制阶段也可更聚焦在 **登录之后的业务操作**。 |
